...
### Frontend
*React:*  
The frontend is the interface that is viewed in the browser by the end user. It operates on the client side and is built using React, a JavaScript library for creating user interfaces. It should provide logical and well defined functionality allowing the user to interact with the application and its backend assets. The typical React front end is what is known as a Single Page Application (SPA) and is self-contained and stored in the browsers local memory in whole. Because all components are locally accessible it does not need to make network requests or re-render to access its various views. Any dynamic data for the application is transferred between itself and the backend. Additionally, because it is a Single Page Application, requests to different site url endpoints are not transferred over a network but are handled by an internal react router which mounts and unmounts components to create the requested page views. 
### Backend
...
In reflecting on my Portfolio assignment I can identify areas of strengths, but more importantly knowledge and experience that was lacking to produce the most effective product. This assignment was particularly challenging for me as my move from the US to Australia occured at the same time. 
...
Every language may implement type coercion differently making it important to understand any idiosyncrasies to avoid unforeseen behavior and frustration. 
