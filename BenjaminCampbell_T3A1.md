...
### Frontend
*React:*  
The frontend is the interface that is viewed in the browser by the end user. It operates on the client side and is built using React, a JavaScript library for creating user interfaces. It should provide logical and well defined functionality allowing the user to interact with the application and its backend assets. One typical React front end paradigm is Single Page Application (SPA) design. The web app is downloaded fully and relies on the implementation of a virtual DOM which it uses to render or re-render targeted components for any view. Because all components are locally accessible it does not need to make network requests or re-render entire pages to display views. Any stored or dynamic data for the application is transferred between itself and the backend. Additionally, because it is a Single Page Application, requests to different site url endpoints are not transferred over a network but are handled by an internal react router which mounts and unmounts components to create the requested page views. All of this adds up to an extremely performant user experience with very little, if any, downtime when displaying a web apps content.
